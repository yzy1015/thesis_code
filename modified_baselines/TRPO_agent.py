...
class TRPO_agent_new(object):
    index2 = 0
    def __init__(self, a_name,env,par):
        ...
        self.max_episodes=0
        self.max_iters=1  # time constraint
        self.max_epi_avg = 100000000000
        self.max_iters = 1 
    ...
    # ----------------------------------------
        ...
        self.pi = par.policy_func(a_name, self.ob_space, self.ac_space)
        self.oldpi = par.policy_func("oldpi"+a_name, self.ob_space, self.ac_space)
        ...
        self.initial_traj(self.timesteps_per_batch)
    ...
    def initial_traj(self, horizon):
        # Initialize state variables
        self.t_traj = 0
        #self.ac_traj = env.action_space.sample() move to beginning
        self.new_traj = True
        self.rew_traj = 0.0
        #self.ob_traj = env.reset() move to beginning
    ...
    def yield_func(self):
        yield {"ob" : self.obs_traj, "rew" : self.rews_traj, "vpred" : self.vpreds_traj, "new" : self.news_traj, "ac" : self.acs_traj, "prevac" : self.prevacs_traj, "nextvpred": self.vpred_traj * (1 - self.new_traj), "ep_rets" : self.ep_rets_traj, "ep_lens" : self.ep_lens_traj}
    
    def traj_segment_generator(self, ac_t, vp_t, obs_t, horizon):
        self.prevac_traj = self.ac_traj
        self.ac_traj = ac_t
        self.vpred_traj = vp_t #pi.act(stochastic, self.ob_traj)  #########################################
        self.ob_traj = obs_t
        ...
        if temp_result:
            return self.yield_func()
        else:
            return None
        #self.ob_traj, self.rew_traj, self.new_traj, _ = env.step(self.ac_traj) ############################
    
    # after step
    def obtain_reward(self, rew_traj, new_traj):         
        temp_learn = False
        self.rew_traj = rew_traj
    ...
    def action_st(self, obs, learn_temp = True):
        ac, vpred = self.pi.act(True, obs)
        if learn_temp:
            self.temp_seg_gen = self.traj_segment_generator(ac, vpred, obs, self.timesteps_per_batch)
        return ac
